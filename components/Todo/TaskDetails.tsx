            ...
            data?.userTask?.map((todo:any)=>(
                <div key={todo?.id} className='flex gap-3 flex-col'>
                    <details className='w-full relative'>
                        <summary className='text-2xl font-mono cursor-pointer flex justify-evenly items-center'>
                            {todo.name}
...
function Modal({data,setAddTask}:any){
    ...
    return(
        <div ref={ModalRef} onClick={(e:any)=>CloseModal(e)} className='w-screen h-screen fixed bg-[#88662D] inset-0 opacity-90 backdrop-blur-2xl flex justify-center items-center'>
            <div className=' bg-richblack-900 rounded-lg w-2/6 p-9 flex items-center gap-5 shadow-[0px_0px_75px_75px_#FFE83D] '>
                <UnderLine><h2 className='text-caribbeangreen-25 text-xl font-serif font-extrabold italic '>Add new Task: </h2> </UnderLine>
                <div>
